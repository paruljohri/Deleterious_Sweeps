...
#Get the B value:
t <- read.table(paste("/home/pjohri/DelSweeps/", s_folder,"/pi_neutral.txt", sep=""),h=T)
pi_neu_int <- mean(t$pi_intergenic)
B <- pi_neu_int/(4*N*u)
...
Pfix <- function(x) {(x*B)/(exp(2*N*B*x) - 1)}

#For neutral coding:
t <- read.table(paste("/home/pjohri/DelSweeps/", s_folder,"/time_to_fixation_m1.txt", sep=""),h=T)
t$time_2Ngen_BGS <- t$time_gen/(2*N*B)
t_mean <- aggregate(t$time_2Ngen_BGS~t$repID, FUN=mean)
t_time <- rbind(t_time, c("0-1syn", mean(t_mean[,2]), quantile(t_mean[,2], 0.25)))


t_num_sub <- aggregate(t$time_2Ngen_BGS~t$repID, FUN=length)
t_prob_fix <- t_num_sub[,2]/(2*N*u*tot_exon_len*num_gen*f0)
s_num_subs <- length(t$time_2Ngen_BGS)
s_prob_fix_pooled <- s_num_subs/(num_reps*2*N*u*tot_exon_len*num_gen*f0)
...
s_num_subs <- length(t$time_2Ngen_BGS)
s_prob_fix_pooled <- s_num_subs/(num_reps*2*N*u*tot_nonexon_len*num_gen)
...
while (gamma1 < 10){
	...
	t$time_2Ngen_BGS <- t$time_gen/(2*N*B)
	s_num_subs <- length(t$time_2Ngen_BGS[which(t$gamma_BGS>-1.0*gamma2 & t$gamma_BGS <= -1.0*gamma1)])
        print (s_num_subs)

	if (s_num_subs>0){
		t_mean <- aggregate((t$time_2Ngen_BGS[which(t$gamma_BGS>-1.0*gamma2 & t$gamma_BGS <= -1.0*gamma1)])~t$repID[which(t$gamma_BGS>-1.0*gamma2 & t$gamma_BGS <= -1.0*gamma1)], FUN=mean)
		t_time <- rbind(t_time, c(paste(gamma1, "-", gamma2, sep=""), mean(t_mean[,2]), quantile(t_mean[,2], 0.25)))
		
		t_num_sub <- aggregate(t$time_2Ngen_BGS[which(t$gamma_BGS>-1.0*gamma2 & t$gamma_BGS <= -1.0*gamma1)]~t$repID[which(t$gamma_BGS>-1.0*gamma2 & t$gamma_BGS <= -1.0*gamma1)], FUN=length)
		...
		t_pfix <- rbind(t_pfix, c(paste(gamma1, "-", gamma2, sep=""), MeanProb, s_num_subs, s_prob_fix_pooled, mean(t_prob_fix, na.rm=T), sd(t_prob_fix)))
		}
	else{
...
write.table(t_time, file=paste("/home/pjohri/DelSweeps/", s_folder,"/time_to_fixation_BGS.stats", sep=""), append=F, sep='\t', row.names=F)
write.table(t_pfix, file=paste("/home/pjohri/DelSweeps/", s_folder,"/prob_of_fixation_BGS.stats", sep=""), append=F, sep='\t', row.names=F)
